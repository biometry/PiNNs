...
#        v6.append(val_loss[5][i])
    ...
    pd.DataFrame({"f1": v1, "f2": v2, "f3":v3, "f4": v4, "f5": v5}).to_csv(f'~/physics_guided_nn/results/mlpDA{da}_vloss.csv')
    ...
    #print("LOSS", tloss)
    pd.DataFrame({"f1": t1, "f2": t2, "f3":t3, "f4": t4, "f5": t5}).to_csv(f'~/physics_guided_nn/results/mlpDA{da}_trainloss.csv')
