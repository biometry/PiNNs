...
via <- function(data_use, gridsize = 200){
  ...
  for (v in variables){
    ...
    output = matrix(nrow = gridsize, ncol = 4)
    colnames(output) = c("mar", "jun", "sep", "dec")
    ...
    sep = with(hyytiala_test, hyytiala_test[(date >= "2008-09-13" & date <= "2008-09-28"),])[,c(3:11)]
    ...
    write.csv(output, paste0("~/Projects/physics_guided_nn/results/preles_", data_use, "_", v, "_via_conditional.csv"))
  }
  return(output)
}
