...
def train(hpar, model_design, X, Y, Xn, data_dir='./', pt=None):
    # initialize data
    # hyperparameters
    n_epoch = hpar['epochs']
    ...
    for i in range(n_epoch):
        ...
        for step, train_data in enumerate(train_loader):
            ...
            # forward
            p, y_hat = model(xt, xnt)
            #p = model(xt, xnt)
            #loss = torch.mean(torch.square(criterion(y_hat, yt) + 2*criterion(p, P)))
            loss = criterion(p, P)
            print('loss', loss)
            # backward
            loss.backward()
        ...
        with torch.no_grad():
            for step, val_sample in enumerate(val_loader):
                ...
                pv, y_hat_val = model(x_val, xn_val)
                #loss = torch.mean(torch.square(criterion(y_hat_val, y_hat) + 2*criterion(pv, Pv)))
