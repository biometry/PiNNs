...
o1 <- PRELES(s1$PAR, s1$TAir, s1$VDP, s1$Precip, s1$CO2, s1$fAPAR, DOY=s1$DOY, p=defaults,
...
o2 <- PRELES(s1$PAR, s1$TAir,  s1$D, s1$Precip, s1$CO2, s1$fAPAR, DOY=s1$DOY, p=decid,
             returncols=c("GPP", "ET",  "fS", "fW", "SW",
                          "Canopywater", "SOG", "S"), pft="notconifer",
             LOGFLAG=0)

measuredGPP = s1$GPPobs
measuredGPP[measuredGPP < -998] <- NA
plot(s1$DOY, s1$ETobs)
points(s1$DOY, o1$ET, col=2)
points(s1$DOY, o2$ET, col=3)

write.csv(s1, "testdat.csv")
