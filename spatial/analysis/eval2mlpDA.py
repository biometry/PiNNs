...
def eval2mlpDA(data_use="full", da=1, exp = "exp2"):
    ...
    pd.DataFrame.from_dict(performance).to_csv(f'./results/2mlpDA{da}_eval_{data_use}_performance.csv')
    pd.DataFrame.from_dict(preds_test).to_csv(f'./results/2mlpDA{da}_eval_preds_{data_use}_test.csv')
