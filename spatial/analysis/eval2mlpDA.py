...
def eval2mlpDA(data_use="full", da=1, exp = "exp2", N=5000):
    ...
    print(td, se, ae)
    pd.DataFrame.from_dict(td).to_csv(f'spatial/results/2mlpDA_{data_use}_eval_tloss.csv')
    pd.DataFrame.from_dict(se).to_csv(f'spatial/results/2mlpDA_{data_use}_eval_vseloss.csv')
    pd.DataFrame.from_dict(ae).to_csv(f'spatial/results/2mlpDA_{data_use}_eval_vaeloss.csv')
    ...
    pd.DataFrame.from_dict(performance).to_csv(f'spatial/results/2mlpDA{da}_eval_{data_use}_performance.csv')
    pd.DataFrame.from_dict(preds_test).to_csv(f'spatial/results/2mlpDA{da}_eval_preds_{data_use}_test.csv')
