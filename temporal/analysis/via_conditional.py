...
def predict(test_x, test_y, m, data_use, yp, xt_test, current_dir='', N=5000, da=1):
    ...
    for i in range(4):
        ...
        if m =='mlpDA':
            model.load_state_dict(torch.load(''.join((data_dir, f"{m}_pretrained_{data_use}_{N}_{da}_trained_model{i}.pth"))))
        else:
...
def via(data_use, model, prediction_scenario, current_dir =''):
    ...
    if model == 'res2':
        ...
        x_tr, mn, std = utils.standardize(yptr, get_p=True)
        x_te = utils.standardize(ypte, [mn, std])
...
if __name__ == '__main__':
    ...
    via(args.d, args.m, prediction_scenario = 'temporal')
