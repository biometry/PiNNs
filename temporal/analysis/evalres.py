...
def evalres(data_use='full'):
    ...
    d = pd.read_csv(f"temporal/results/NresHP_{data_use}.csv")
    ...
    pd.DataFrame({"f1": t1, "f2": t2, "f3":t3, "f4":t4}).to_csv(f'temporal/results/res_trainloss_{data_use}.csv')
    v1 = []
    v2 = []
    v3 = []
    v4 = []
    ...
    pd.DataFrame({"f1": v1, "f2": v2, "f3":v3, "f4":v4}).to_csv(f'temporal/results/res_vloss_{data_use}.csv')
    ...
    pd.DataFrame.from_dict(performance).to_csv(f'temporal/results/res_eval_{data_use}_performance.csv')
    pd.DataFrame.from_dict(preds_tr).to_csv(f'temporal/results/res_eval_preds_{data_use}_train.csv')
    pd.DataFrame.from_dict(preds_te).to_csv(f'temporal/results/res_eval_preds_{data_use}_test.csv')
