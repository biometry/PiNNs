...
def eval3mlp(data_use='full'):
    ...
    pd.DataFrame.from_dict(performance).to_csv(f'./results/3mlp_eval_{data_use}_performance.csv')
    pd.DataFrame.from_dict(preds_test).to_csv(f'./results/3mlp_{data_use}_eval_preds_test.csv')
